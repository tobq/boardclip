...
// In-memory index of the pool's hashes per baseDir, seeded ONCE from a readdir
// of objects/ (one syscall for the whole pool). The previous existsSync-per-item
// check was a ~10k-stat storm on every snapshot of a mature history (~0.8s of
// the 0.9s a no-change snapshot cost, measured 2026-09-02). GC invalidates the
// index for its baseDir so a re-referenced object is re-written, never assumed.
const poolIndex = new Map();
function knownHashes(baseDir) {
  let set = poolIndex.get(baseDir);
  if (!set) {
    set = new Set();
    let names = [];
    try { names = fs.readdirSync(objectsDir(baseDir)); } catch {}
    for (const name of names) {
      if (OBJECT_RE.test(name)) set.add(name.slice(0, -'.json'.length).toLowerCase());
    }
    poolIndex.set(baseDir, set);
  }
  return set;
}
...
function putObject(baseDir, value) {
  ...
  const known = knownHashes(baseDir);
  if (!known.has(hash)) {
    blobStore.ensureDir(objectsDir(baseDir));
    blobStore.atomicWriteFile(objectPath(baseDir, hash), json);
    known.add(hash);
  }
  return hash;
}
...
function gcObjects(baseDir, survivors) {
  ...
  poolIndex.delete(baseDir); // objects vanished: re-seed lazily on the next putObject
  return freed;
}
...
// Object GC reads every surviving manifest, so on the save hot path it must not run
// on every prune. Throttle it: the cheap part (manifest eviction) always runs; the
// full-scan GC + size enforcement runs only when something was actually evicted (so
// orphans are possible) and the throttle has elapsed, OR when the byte cap is exceeded.
// Orphaned objects between GCs are tiny (a changed item each) and harmless.
const lastGcByBase = new Map();
...
function pruneBackups(baseDir, { maxAgeMs, maxBytes, maxManifests, now = Date.now(), gcMinIntervalMs = 0 } = {}) {
  let survivors = listSnapshots(baseDir);

  // 1. age + count eviction (planRetention is oldest-first LRU by mtime). Cheap:
  //    listSnapshots only stats files, and eviction is stat+unlink — no manifest reads.
  ...
  const overSize = maxBytes != null && totalBytes(baseDir) > maxBytes;
  const gcDue = (now - (lastGcByBase.get(baseDir) || 0)) >= gcMinIntervalMs;
  if (!((evictedPaths.size > 0 && gcDue) || overSize)) return;

  // 2. GC objects no longer referenced by a surviving manifest.
  gcObjects(baseDir, survivors);
  lastGcByBase.set(baseDir, now);

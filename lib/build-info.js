...
function getBuildInfo(baseDir) {
  ...
  // Only TRACKED changes count as "dirty" (matches update.bat's guard). Untracked
  // runtime/data files (clipboard-RECOVERED/, PRE-RECOVERY backups, edit-archive, etc.)
  // must NOT mark the checkout dirty, or they silently block the auto-updater
  // (canAutoUpdate refuses a dirty checkout) after any recovery leaves files behind.
  const dirty = !!runGit(baseDir, ['status', '--porcelain', '--untracked-files=no']);

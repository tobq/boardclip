...
function applyTextEdit(items, options = {}) {
  ...
  if (newText === originalText) return { changed: false, reason: 'unchanged', tombstoneIds };
  ...
  if (existingIndex >= 0) {
    ...
    return {
      changed: true,
      reason: canUpdateCurrent ? 'merged' : 'conflict_merged',
      item: existing,
      tombstoneIds,
    };
  }
  ...
  if (canUpdateCurrent) {
    ...
    if (oldId && oldId !== editedId) tombstoneIds.push(oldId);
    dedupeNumpadSlots(history);
    return { changed: true, reason: 'updated', item: current, tombstoneIds };
  }

  ensureItemId(edited);
  history.unshift(edited);
  dedupeNumpadSlots(history);
  return { changed: true, reason: 'conflict_created', item: edited, tombstoneIds };
}

...
// `handleRequest(method, payload, { signal })` by main.js, so all mutation logic
// stays in the app and is never duplicated here.
//
// Two liveness rules protect a request that waits on the user (approval modal):
// - `signal` aborts when the caller's socket closes (it timed out or died), so
//   the app can drop the prompt instead of running the action for nobody.
// - A caller that sent `keepalive` in its envelope gets `{id, pending: true}`
//   every keepaliveMs while the request runs; its inactivity timeout then waits
//   for as long as the user deliberates. Older helpers that never set the flag
//   see exactly one line, the response, as before.
...
class ControlServer {
  constructor({ pipePath, secret, handleRequest, onError, windowMs, keepaliveMs }) {
    this.pipePath = pipePath;
    this.secret = secret;
    this.handleRequest = handleRequest;
    this.onError = onError || (() => {});
    this.windowMs = windowMs || hmacAuth.DEFAULT_WINDOW_MS;
    this.keepaliveMs = keepaliveMs || 10_000;
    this.server = null;
  }
  ...
  async _handleLine(conn, line) {
    ...
    if (req.keepalive) {
      pulse = setInterval(() => {
        if (ac.signal.aborted || conn.destroyed) return;
        this._respond(conn, id, { pending: true });
      }, this.keepaliveMs);
    }
    try {
      const result = await this.handleRequest(reqPath || method, payload, { signal: ac.signal });

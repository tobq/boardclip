...
const search = require('../site/shared/clip-search');
...
// A clip carries a format when the value is inline OR externalized to a blob.
// Checked without loading the blob: this is availability, not content.
function hasRichPayload(item, field) {
  if (!item || item.type === 'image') return false;
  const names = model.blobFieldNames(field);
  return !!(item[names.value] || item[names.ref] || item[names.hash]);
}
...
function clipView(item, { sharedSet, previewChars = DEFAULT_PREVIEW_CHARS, forcePreview = false } = {}) {
  ...
  // Advertise that formatting exists WITHOUT shipping it. The raw HTML/RTF is
  // large and low-value to a model, but knowing a clip is rich is useful.
  if (hasRichPayload(item, 'html')) base.has_html = true;
  if (hasRichPayload(item, 'rtf')) base.has_rtf = true;
  return base;
}
...
// A boundary-aware search doc: the SAME normalized shape the shared engine ranks/filters,
// but with title/groups HIDDEN unless the clip is shared (or forceAll after approval). So the
// engine's grammar (title:/since:/is:/…) works while the allowlist boundary is preserved — a
// `title:` scope on a non-shared clip can never match unless the user approved all-scope.
function clipToSearchDoc(item, { sharedSet, forceAll = false } = {}) {
  const visible = forceAll || isShared(item, sharedSet);
  const isImage = item && item.type === 'image';
  const body = isImage ? String(item && item.image || '') : String(item && (item.text != null ? item.text : item.textPreview) || '');
  return {
    id: model.itemKey(item),
    type: isImage ? 'image' : 'text',
    title: visible ? model.titleOf(item) : '',
    body,
    groups: visible ? model.groupsOf(item) : [],
    numpad: model.numpadSlotOf(item),
    pinned: model.isPinned(item),
    ...
    url: !isImage && /^(?:[a-z][a-z0-9+.-]*:\/\/|www\.)[^\s]+$|^[a-z0-9-]+(?:\.[a-z0-9-]+)+(?:\/[^\s]*)?$/i.test(body.trim()),
    rich: !isImage && !!(item.html || item.htmlRef || item.htmlHash || item.rtf || item.rtfRef || item.rtfHash),
  };
}

// Search. Shared matches return previews; non-shared matches are counted only (the caller
// decides whether to escalate to an approval-gated full search). Delegates parsing +
// matching to the SHARED engine (site/shared/clip-search) so the MCP tool understands the
// SAME syntax as the app popup — `title:`, `group:`, `is:pinned`, `since:7d`, `-word`, etc.
// Plain-text queries behave exactly as before (backward compatible).
function searchClips(history, settings, {
  ...
  for (const item of items) {
    const shared = isShared(item, sharedSet);
    const doc = clipToSearchDoc(item, { sharedSet, forceAll: scope === 'all' });
    if (!search.matchDoc(doc, parsed, { regex, now })) continue;
    if (shared) {

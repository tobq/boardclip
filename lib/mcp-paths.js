...
// BOARDCLIP_MCP_PIPE_TAG overrides it (test seam): an isolated QA instance can
// run its own control channel beside the user's live app instead of colliding
// on the per-user pipe with EADDRINUSE.
function userTag() {
  if (process.env.BOARDCLIP_MCP_PIPE_TAG) return String(process.env.BOARDCLIP_MCP_PIPE_TAG).replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 24) || 'u';
  try {

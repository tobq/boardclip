...
// test/numpad-paste.test.js). main.js wires the real Electron clipboard and the
// platform paste primitive. There is a SINGLE delivery mechanism — put the item
// on the clipboard, synthesize Ctrl/Cmd+V, safe-restore the previous clipboard —
// which is the SAME primitive the panel-click paste uses. Numpad therefore can
// never diverge into typing keystrokes (an earlier keystroke-injection mode
// turned newlines into Enter presses and fired unintended sends; it was removed).
...
function createQuickPaster(deps) {
  ...
  async function timedDeliver({ item, paste: doPaste, cfg, trace }) {
    ...
    return { verified: verify.ok, verifyAttempts: verify.attempts, pasteResult, restoreDelay };
  }
  ...
  async function runOne(item, opts) {
    ...
    try {
      ...
      backup = snapshot();
      const delivered = await timedDeliver({ item, paste: doPaste, cfg, trace });
      outcome = { ok: true, ...delivered };

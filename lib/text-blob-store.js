...
function writeTextBlob(baseDir, text, hash) {
  ...
  if (!filePath) return '';
  if (!fs.existsSync(filePath)) atomicWriteFile(filePath, String(text || ''));
  return ref;
}
...
function copyMissingFiles(fromDir, toDir) {
  ...
  for (const name of names) {
    const safe = safeTextRef(name);
    if (!safe) continue;
    const source = path.join(fromDir, safe);
    const dest = path.join(toDir, safe);
    try {
...
function syncTextBlobs(localDir, remoteDir) {
  copyMissingFiles(remoteDir, localDir);
  copyMissingFiles(localDir, remoteDir);
}

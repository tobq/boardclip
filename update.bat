...
if defined NEED_INSTALL (
  ...
  call npm install
  rem npm may rewrite the tracked lockfile; a dirty lock would block the NEXT update,
  rem so restore the repo copy - node_modules is already installed. Must be rem, not ::
  rem a :: label inside a ( ) block aborts the whole script with a parse error.
